...
while(i<s1.n && j<s2.n)
{
if(s1.terms[i].r<s2.terms[j].r ||

s1.terms[i].r==s2.terms[j].r && s1.terms[i].c<s2.terms[j].c)

s3.terms[k++]=s1.terms[i++];
else if(s1.terms[i].r>s2.terms[j].r ||
...
if(k<s3.n) continue;
for(k=k-1;k>=0 && (t.r<s3.terms[k].r || (t.r==s3.terms[k].r
